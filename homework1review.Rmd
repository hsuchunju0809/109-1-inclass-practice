...
```{r ans13}
# G5TimeParsed
G5TimeParsed <- mdy_hm(Game5$Date,"2020",Game5$StartTime,
        tz="America/New_York")
G5TimeParsed
```
...
```{r ans15}
# ParisLouis
with_tz(`TpeJason`,tz="Europe/Paris")
`ParisLouis` <- minutes(15)
...
sum(as.integer(fct_chNumbers))-> sumChNumbers
sumChNumbers
```
...
sumChNumbersWith0 <-
sum(as.integer(fct_ChNumbersWith0))-500
sumChNumbersWith0
```
...
```{r ans32}
# dearStudent
deadlineUTC <- format_ISO8601(with_tz(student$deadline,tz="UTC"))
dearStudent <- paste0("Dear ",student$name,":\n\n貼心提醒您，作業上傳截止時間是",deadlineUTC,"。\n\n你的貼心學伴巴西利bot silly")
cat(dearStudent)
```
