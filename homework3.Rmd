...
```{r ans13}
# Users_E
set_grade16 <- c(1,2,3,4,5,6)
pick_grade16 <- Info_UserData$user_grade %in% set_grade16
pick_na <- is.na(Info_UserData$gender)
Users_E <- Info_UserData$uuid[pick_grade16&pick_na]
Users_E
```
...
set_grade_odd <- c(1,3,5)
pick_grade_odd <- Info_UserData$user_grade %in% set_grade_odd
Users_E_in_odd_grades <- Info_UserData$uuid[pick_grade_odd&pick_na&pick_grade16]
Users_E_in_odd_grades
```
...
```{r ans15}
# Users_E_Not_Metro 
set_Metro <- c("tp","ntpc","ty","tc","tn","kh")
pick_fromMetro <- Info_UserData$user_city %in% set_Metro
pick_not_from_Metro <- !pick_fromMetro
Users_E_Not_Metro<- Info_UserData$uuid[pick_not_from_Metro&pick_na&pick_grade16]
Info_UserData$uuid[pick_not_from_Metro&pick_na&pick_grade16]
```
...
所以 __請延續 1.5 題（即需要延續 1.2, 1.3 的條件）__，針對那非六些都的使用者們，找到那些推測受城鄉差距負面影響較小的使用者群，一樣是找到使用者的 Id，儲存為`Users_E_Not_Metro_Buffer`。

```{r ans16}
# Users_E_Not_Metro_Buffer
pick_self_coach <- Info_UserData$is_self_coach
pick_first_login_date <- Info_UserData$first_login_date_TW == lubridate::ymd("2018-08-30")
Users_E_Not_Metro_Buffer <- Info_UserData$uuid[(pick_not_from_Metro&pick_na&pick_grade16&pick_self_coach)|(pick_not_from_Metro&pick_na&pick_grade16&pick_first_login_date)]
Users_E_Not_Metro_Buffer
...
dppSuppurtRate_sorted<- sort(dppSupportRate)
levelSequence <- names(dppSuppurtRate_sorted)
levelSequence
```
...
election2020[["鄉(鎮、市、區)別"]] <-
  factor(
    election2020$`鄉(鎮、市、區)別`,
    levels=rev(c("烏來區","新店區","永和區","中和區","瑞芳區","汐止區","深坑區", "淡水區","雙溪區","三芝區","林口區","石門區","平溪區","萬里區","三峽區","八里區","土城區","板橋區","金山區","泰山區","鶯歌區","樹林區","五股區","新莊區","石碇區","三重區","坪林區","蘆洲區",
"貢寮區"))
  )
election2020[["鄉(鎮、市、區)別"]]
```
